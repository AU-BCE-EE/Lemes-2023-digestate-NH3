...
```{r}
lmods <- dat[, .(int = coef(lm(log10(oav) ~ time))[1], 
                 slope = coef(lm(log10(oav) ~ time))[2]), 
             by = .(exper, treat, rep)]
```
...
```{r}
modexp1 <- lm(slope ~ treat, data = lmods, subset = exper == 'Experiment 1')
summary.aov(modexp1)
summary(modexp1)
```

Both D-CM and CC-CM have lower slope than reference in experiment 1.
**Use these results in paper.**

Experiment 2 now

```{r}
modexp2 <- lm(slope ~ treat, data = lmods, subset = exper == 'Experiment 2')
summary.aov(modexp2)
summary(modexp2)
```
...
```{r}
modinit1 <- lm(log10(oav) ~ treat * exper, data = dat, subset = int == 1)
summary.aov(modinit1)
summary(modinit1)
```
...
```{r}
modinit2 <- lm(log10(oav) ~ treat + exper, data = dat, subset = int == 1)
summary.aov(modinit2)
summary(modinit2)
```
